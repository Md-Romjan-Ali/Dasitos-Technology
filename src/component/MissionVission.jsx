                        ...
                        {missionVisionValues.map((item, index) => {
                            ...
                            return (
                                <motion.div
                                    ...
                                    whileHover={{ y: -8 }}
                                    className="group relative rounded-3xl border border-slate-800 bg-slate-900/40 p-8 shadow-2xl backdrop-blur-xl transition-all duration-300 hover:border-blue-500/50 hover:bg-slate-900/70 hover:shadow-blue-500/10 flex flex-col justify-between"
                                >
                                    <div className="space-y-6">
                                        {/* Top Icon & Badge */}
                                        <div className="flex items-center justify-between">
                                            <div className="p-3.5 rounded-2xl bg-blue-500/10 border border-blue-500/20 text-blue-400 group-hover:bg-blue-600 group-hover:text-white transition-all duration-300">
                                                <Icon className="size-6" />
                                            </div>
                                            <span className="text-slate-400 text-xs font-medium bg-slate-800/60 px-3 py-1 rounded-full border border-slate-700/50">
                                                {item.tag}
                                            </span>
                                        </div>
                                    ...
                                    {/* Highlight Footer */}
                                    <div className="pt-6 mt-6 border-t border-slate-800/80 flex items-center gap-2 text-xs font-semibold text-blue-400">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        {segments.map((segment, index) => (
                            <motion.div
                                ...
                                className="group flex items-center gap-3.5 p-4 rounded-2xl border border-slate-800 bg-slate-900/30 backdrop-blur-md hover:bg-slate-800/60 hover:border-blue-500/40 transition-all duration-300 cursor-default"
                            >
                                <div className="p-2 rounded-xl bg-blue-500/10 text-blue-400 border border-blue-500/20 group-hover:bg-blue-600 group-hover:text-white transition-all duration-300 shrink-0">
                                    <FiBriefcase className="size-4" />
                                </div>
                                <span className="text-slate-200 group-hover:text-white font-medium text-sm sm:text-base transition-colors">
                                    {segment}
                                </span>
                            </motion.div>
                        ))}
